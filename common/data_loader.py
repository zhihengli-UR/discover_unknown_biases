...
SHAPE_ATTRIBUTE_INDEX_PER_DATASET_DICT = {'smallnorb': (0, 5)}
NORMALIZE_INDICES_PER_DATASET_DICT = {'smallnorb': [1, 2, 3]}


def preprocess_label(dataset_name, label):
    assert dataset_name == 'smallnorb'
    shape_index, num_shape = SHAPE_ATTRIBUTE_INDEX_PER_DATASET_DICT[dataset_name]
    float_label = label.copy().astype(np.float32)
    float_label[:, shape_index] = label[:, shape_index] < int(num_shape / 2)

    # normalize remaining attributes
    for attribute_idx in NORMALIZE_INDICES_PER_DATASET_DICT[dataset_name]:
        old_values = label[:, attribute_idx]
        float_label[:, attribute_idx] = old_values / old_values.max()
...
class DisentanglementLibDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        return torch.from_numpy(np.moveaxis(output, 2, 0)).float(), torch.from_numpy(factors).flatten()
