...
class StyleGANGenerator(BaseGenerator):
    ...
    def convert_tf_model(self, test_num=10):
        ...
        self.logger.info(
            f'Loading tensorflow model from `{self.tf_model_path}`.')
        ...
        for pth_var_name, tf_var_name in self.model.pth_to_tf_var_mapping.items():
            if 'ToRGB_lod' in tf_var_name:
                ...
                tf_var_name = tf_var_name.replace(
                    f'{lod}', f'{lod - lod_shift}')
            if tf_var_name not in tf_vars:
                self.logger.debug(f'Variable `{tf_var_name}` does not exist in '
                                  f'tensorflow model.')
                continue
            self.logger.debug(
                f'  Converting `{tf_var_name}` to `{pth_var_name}`.')
        ...
        self.logger.info(
            f'Average distance is {total_distance / test_num:.6e}.')
    ...
    def sample(self, num, latent_space_type='Z'):
        ...
        if latent_space_type == 'Z':
            ...
            latent_codes = self.synthesize(torch.tensor(z).to(self.run_device), latent_space_type='Z', return_w_only=True)
            assert latent_codes.shape == (num, self.w_space_dim)
        elif latent_space_type == 'WP':
            latent_codes = np.random.randn(
                num, self.num_layers, self.w_space_dim)
        else:
            raise ValueError(
    ...
    def preprocess(self, latent_codes, latent_space_type='Z'):
        ...
        if latent_space_type == 'Z':
            ...
            latent_codes = latent_codes.reshape(-1,
                                                self.num_layers, self.w_space_dim)
        else:
            raise ValueError(
                f'Latent space type `{latent_space_type}` is invalid!')
