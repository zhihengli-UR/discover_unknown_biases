...
from models.factorvae import factorvae_loss_fn, factorvae_init, factorvae_D_loss_fn
...
class VAE(BaseDisentangler):
    ...
    def loss_fn(self, input_losses, **kwargs):
        ...
        if c.FACTORVAE in self.loss_terms:
            output_losses['vae_tc_factor'] = factorvae_loss_fn(
                self.w_tc, self.PermD, **kwargs)
    ...
    def train(self):
        while not self.training_complete():
            ...
            for internal_iter, (x_true1, label1) in enumerate(self.data_loader):
                ...
                try:
                    x_true2, label2 = next(self.iter_dataloader)
                except:
                ...
                if not c.FACTORVAE in self.loss_terms:
                    self.optim_G.zero_grad()
                    losses[c.TOTAL_VAE].backward()
                    self.optim_G.step()
                else:
                    discriminator_tc_loss = factorvae_D_loss_fn(self.model, self.PermD, self.optim_PermD, self.ones, self.zeros,
                                        z=params['z'], x_true2=x_true2, label2=label2)
                    total_loss = discriminator_tc_loss + losses[c.TOTAL_VAE]
                    self.optim_G.zero_grad()
                ...
                vae_loss_sum += losses[c.TOTAL_VAE].item()
