...
class BaseDisentangler(object):
    def __init__(self, args):
        ...
        self.aicrowd_challenge = False

        if not args.bias_gen:
            ...
            from biased_dataset.disentanglement_lib_datasets import BiasedDisentanglementLibDataset
            transform = transforms.Compose([
                transforms.Resize((args.image_size, args.image_size)),
                transforms.ToTensor()])
            dataset = BiasedDisentanglementLibDataset(args.dset_name, transform, args.target_attr_index, args.bias_attr_index, args.bias_degree, seed=args.seed)
            self.data_loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=not args.test, drop_last=not args.test, pin_memory=args.pin_memory,
                                          num_workers=args.num_workers)

        self.iter_dataloader = iter(self.data_loader)
        ...
        if not args.test:
            ...
            self.pbar = tqdm(total=self.max_iter, dynamic_ncols=True) if args.use_pbar else None
        else:
            self.pbar = tqdm(self.num_batches, dynamic_ncols=True) if args.use_pbar else None
        ...
        # Checkpoint
        self.ckpt_dir = os.path.join(args.ckpt_dir, 'synthetic/generative_model', args.name)
        if args.save_ckpt:
    ...
    def log_save(self, **kwargs):
        ...
        if is_time_for(self.iter, self.ckpt_save_iter) and self.args.save_ckpt:
    ...
    def step(self, loss):
        ...
        if self.pbar is not None:
            self.pbar.update(1)
        desc = ' '.join(['{}: {:.4f}'.format(k, v) for k, v in loss.items()])
        if self.pbar is not None:
